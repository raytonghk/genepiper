...
shinyUI(
  fluidPage(
    ...
    tags$script(HTML("window.onload = function() { parent.postMessage('importBiomLoaded', '*') }")),
    ...
    fluidRow(
      wellPanel(
        class = "desc-box",
        h5("Imported biom file was used to construct the phyloseq object and store in the genepiper."),
        h5("If following tables were not included in biom file, please supply with table formats:"),
        h5(tags$span(class = "bold", "Taxonomy table"), "should contains all character columns. The first column should be OTU name and each taxonomical rank in individual column."),
        h5(tags$span(class = "bold", "Sample data"), "should contains at least two columns. The first column should be the sample name."),
      ...
      column(
        ...
        wellPanel(
          class = "left-column",
          h4("Biom File:"),
          fileInput("biomFilePath", "File Path"),
          errorOutput("biomFilePathMessage")
        ),
        
        conditionalPanel(
          condition = "output.showTaxTableInput",
          wellPanel(
            class = "left-column",
            tags$div(
        ...
        conditionalPanel(
          condition = "output.showSampleDataInput",
          wellPanel(
            class = "left-column",
            tags$div(
              tags$div(class = "pull-right",
        ...
        conditionalPanel(
          condition = "output.showPhyloTreeInput",
          wellPanel(
            class = "left-column",
            tags$div(
              tags$div(class = "pull-right",
