...
shinyServer(
  function(input, output, session) {
    ...
    output$showTaxTableInput <- reactive(vals$showTaxTableInput)
    outputOptions(output, "showTaxTableInput", suspendWhenHidden = FALSE)
    
    output$showSampleDataInput <- reactive(vals$showSampleDataInput)
    outputOptions(output, "showSampleDataInput", suspendWhenHidden = FALSE)
    
    output$showPhyloTreeInput <- reactive(vals$showPhyloTreeInput)
    outputOptions(output, "showPhyloTreeInput", suspendWhenHidden = FALSE)
    ...
    observe(
      {
        req(input$biomFilePath$datapath)
        isolate(
          {
            vals$biomFilePathMessage <- NULL
            vals$biom <- NULL
            vals$otuTable <- NULL
            vals$taxTable <- NULL
            vals$samData <- NULL
            vals$tree <- NULL
            vals$showTaxTableInput <- FALSE
            vals$showSampleDataInput <- FALSE
            vals$showPhyloTreeInput <- FALSE
            tryCatch(
              {
                vals$biom <- read_biom(input$biomFilePath$datapath)
              },
              error = function(e) {
                vals$biomFilePathMessage <- "Import biom file error!"
              }
            )
          }
        )
    ...
    observe(
      {
        ...
        tryCatch(
          {
            vals$otuTable <- as(biom_data(vals$biom), "matrix") %>%
              as.data.frame() %>%
              rownames_to_column("taxa")
            vals$showPhyloTreeInput <- TRUE
          },
          errror = function(e) {
            vals$biomFilePathMessage <- "OTU table error!"
          }
        )
      }
    )
    ...
    observe(
      {
        ...
        if(validateOtuTable(vals$otuTable)) {
          ...
          vals$biomFilePathMessage <- "Import file format error!"
        }
      }
    )
    
    output$biomFilePathMessage <- renderText(HTML(vals$biomFilePathMessage))
    
    ### Taxonomy Table
    observe(
      {
        req(vals$biom)
        if(is.null(observation_metadata(vals$biom))) {
          vals$showTaxTableInput <- TRUE
        } else {
          tryCatch(
            {
              vals$taxTable <- observation_metadata(vals$biom) %>%
                rownames_to_column("taxa")
            },
            error = function(e) {
              vals$bioFilePathMessage <- "Taxa table in biom file error!"
              vals$showTaxTableInput <- TRUE
            }
          )
        }
    ...
    observe(
      {
        req(vals$biom)
        if(is.null(sample_metadata(vals$biom))) {
          vals$showSampleDataInput <- TRUE
        } else {
          tryCatch(
            {
              vals$samData <- sample_metadata(vals$biom) %>%
                rownames_to_column("sample")
            },
            error = function(e) {
              vals$bioFilePathMessage <- "Sample data in biom file error!"
              vals$showSampleDataInput <- TRUE
            }
          )
        }
